...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT TermName,StudentId FROM tblterms WHERE ClassId= :id order by TermName");
 $stmt->execute(array(':id' => $cid));
 ?><option value="">Select Term </option><?php
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ?>
  <option value="<?php echo htmlentities($row['TermID']); ?>"><?php echo htmlentities($row['TermName']); ?></option>
  <?php
 }
}
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT TermId,ClassId FROM tblresult WHERE TermId=:id1 and ClassId=:id ");
