...
if (!empty($_POST["studclass"])) {
    ...
    $id = $dta[0];
    $id1 = $dta[1];
    $query = $dbh->prepare("SELECT YearId,SemesterId FROM tblresult WHERE StudentId = :id AND YearId = :id1 AND SemesterId = :id2");
    ...
    if ($query->rowCount() > 0) { ?>
        <p>
            <?php
            echo "<span style='color:red'>Result Already Declared for the same semester, term, and year.</span>";
